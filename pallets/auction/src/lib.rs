...
pub mod pallet {
	...
	impl<T: Config> Auction<T::AccountId, BlockNumberFor<T>> for Pallet<T> {
		...
		fn auction_bid_handler(from: T::AccountId, id: AuctionId, value: Self::Balance) -> DispatchResult {
			let mut auction_item: AuctionItem<T::AccountId, BlockNumberFor<T>, BalanceOf<T>> =
				Self::get_auction_item(id.clone()).ok_or(Error::<T>::AuctionDoesNotExist)?;
			ensure!(
	...
	impl<T: Config> Pallet<T> {
		...
		/*
				pub fn upgrade_auction_item_data_v2() -> Weight {
					...
					AuctionItems::<T>::translate(
						|_k, auction_v1: AuctionItemV1<T::AccountId, BlockNumberFor<T>, BalanceOf<T>>| {
							num_auction_items += 1;
							let v2: AuctionItem<T::AccountId, BlockNumberFor<T>, BalanceOf<T>> = AuctionItem {
		...
		*/
		...
		//
		//			AuctionItems::<T>::translate(
		//				|_k, auction_v1: migration_v2::AuctionItem<T::AccountId, BlockNumberFor<T>, BalanceOf<T>>| {
		//					num_auction_item += 1;
		//
		...
		//
		//							let v: AuctionItem<T::AccountId, BlockNumberFor<T>, BalanceOf<T>> = AuctionItem {
		//								item_id: v2_item_id,
		//								recipient: auction_v1.recipient,
		//								initial_amount: auction_v1.initial_amount,
		//								amount: auction_v1.amount,
